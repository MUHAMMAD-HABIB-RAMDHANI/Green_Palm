@extends('layouts.app')
@section('title', 'Catat Rawat - Kastrasi')
        ...
        <div class="card-body">
            ...
            <div class="step-header">
                <div class="step-title">
                    <h2>
                        Perawatan Kastrasi
                        @if(isset($lastData)) <span class="autofill-badge">✨ Auto</span> @endif
                    </h2>
            ...
            <form id="kastrasiForm" action="{{ route('catatan.store') }}" method="POST">
                @csrf
                <input type="hidden" name="jenis_kegiatan" value="kastrasi">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Tanggal Kastrasi <span style="color: red;">*</span></label>
                        <input type="date" name="tanggal_kastrasi" class="form-control {{ isset($lastData) ? 'autofilled' : '' }}" 
                            value="{{ old('tanggal_kastrasi', isset($lastData) ? $lastData->tanggal_kastrasi : now()->format('Y-m-d')) }}" required>
                        @error('tanggal_kastrasi') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Jumlah Pokok Dikastrasi <span style="color: red;">*</span></label>
                        <input type="number" name="jumlah_pokok_dikastrasi" class="form-control" 
                            placeholder="Contoh: 50" value="{{ old('jumlah_pokok_dikastrasi') }}" required>
                        @error('jumlah_pokok_dikastrasi') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addUpahRow = function() {
            ...
            div.innerHTML = `
                <div style="flex: 1;">
                    <input type="text" name="rincian_upah[${counter}][jenis]" class="form-control" placeholder="Jenis (Cth: Upah Kastrasi)">
                </div>
                <div style="flex: 1;">
