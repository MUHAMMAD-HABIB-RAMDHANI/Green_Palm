...
@section('mobile-header')
    <header class="mobile-header-custom">
        <div class="header-left-content">
            {{-- Tombol Kembali --}}
            <a href="{{ route('dashboard.daftar-hama') }}" class="mobile-back-btn">
                ‹
            </a>
...
<div class="hama-wrapper">
    <div class="hama-container">
        ...
        <div class="hama-header">
            <a href="{{ route('dashboard.daftar-hama') }}" class="btn-back">‹</a>
            <div class="header-title">Detail Hama</div>
        </div>

        {{-- Gambar Hama Full Width --}}
        <div class="detail-image-wrapper">
            <img src="{{ $hama->image_url }}" class="detail-image" alt="{{ $hama->name }}"
                 onerror="this.src='https://placehold.co/600x400/e0e0e0/333?text=No+Image'">
        </div>

        {{-- Isi Konten --}}
        <div class="detail-content">
            <h1 class="detail-title">{{ $hama->name }}</h1>
            @if($hama->latin_name)
                <span class="detail-latin">({{ $hama->latin_name }})</span>
            @else
            ...
            <div class="detail-text">
                {{ $hama->description ?? 'Tidak ada deskripsi tersedia.' }}
            </div>
            ...
            <div class="solution-box">
                <div class="solution-text">
                    {{ $hama->solution ?? 'Belum ada data solusi.' }}
                </div>
            </div>
        </div>
