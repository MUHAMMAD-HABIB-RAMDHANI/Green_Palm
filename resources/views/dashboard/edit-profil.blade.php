...
<style>
    ...
    @media (max-width: 768px) {
        ...
        /* --- Style komponen lain di mobile --- */
        .user-info-banner {
            padding: 18px 20px;
            margin-bottom: 30px;
        }

        .user-info-icon { font-size: 28px; }
        .user-info-content h3 { font-size: 16px; }
        .user-info-content p { font-size: 12px; }
        ...
        .form-input, .form-select { padding: 13px 16px; font-size: 14px; }

        .button-section { margin-top: 30px; padding-top: 25px; }
        .btn { padding: 15px; }
        .alert { font-size: 13px; padding: 14px 18px; }
    }
</style>
...
<div class="edit-profile-wrapper">
    <div class="edit-profile-card">
        ...
        {{-- Header Desktop (Akan hilang di mobile karena CSS) --}}
        <div class="card-header">
            <a href="{{ route('dashboard.profil') }}" class="back-button" title="Kembali">
                ‹
            </a>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        <label class="form-label">
                            ...
                            Nama Pengguna <span class="required-mark">*</span>
                        </label>
                        <input type="text" class="form-input" name="username" value="{{ old('username', $user->username) }}" placeholder="Masukkan nama pengguna" required>
                    </div>

                    {{-- Email (Read Only) --}}
                    <div class="form-group full-width">
                        <label class="form-label">
                            <span class="form-label-icon">📧</span> Email
                        </label>
                        <input type="email" class="form-input" value="{{ $user->email }}" readonly title="Email tidak dapat diubah">
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            <span class="form-label-icon">📱</span> No Handphone
                        </label>
                        <input type="text" class="form-input" name="phone" value="{{ old('phone', $user->phone ?? '') }}" placeholder="08xxxxxxxxxx" pattern="[0-9]*" maxlength="15">
                    </div>

                    {{-- Jenis Kelamin --}}
                    <div class="form-group">
                        <label class="form-label">
                            <span class="form-label-icon">⚧️</span> Jenis Kelamin
                        </label>
                    ...
                    <div class="form-group full-width">
                        <label class="form-label">
                            <span class="form-label-icon">🎂</span> Tanggal Lahir
                        </label>
                        <input type="date" class="form-input" name="birth_date" value="{{ old('birth_date', $user->birth_date ?? '') }}" max="{{ date('Y-m-d') }}">
                    </div>
                </div>
...
<div id="toast-container" class="gp-toast-container" aria-live="polite" aria-atomic="true">
    @if($s) <div class="gp-toast gp-toast--success">{{ $s }}</div> @endif
    @if($e) <div class="gp-toast gp-toast--error">{{ $e }}</div> @endif
    @if($errors->any())
        <div class="gp-toast gp-toast--error">
            @if($errors->count() === 1) {{ $errors->first() }}
            @else Terjadi {{ $errors->count() }} kesalahan. @endif
        </div>
    @endif
</div>
